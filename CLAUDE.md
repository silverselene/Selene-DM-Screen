...
- No component library: the widgets are styled with plain `className` strings, inline `style` + CSS variables, and `lucide-react` icons. The old shadcn/ui scaffolding under `src/components/ui/` (and its ~47 Radix/form/chart deps) was **removed as unused** — nothing in the App/widget graph ever imported it. If you want a shadcn primitive, re-add just that component via the shadcn CLI (style: `new-york`) and pull in only the dep it needs.
